...
class ViewController: UIViewController {
   ...
   */
    ...
    @IBOutlet weak var baseTotalInput: UITextField!
    ...
    @IBAction func buttonPress(sender: AnyObject) {
        ...
        func tipCalc() {
            ...
            if let tipPercent = tipInput.text.toInt() {
                
                var tipPercent2:Double = Double(tipPercent) * 0.01
            
            
            
        
            if let baseAmount = baseTotalInput.text.toInt(){
                
                tipAmount = tipPercent2 * Double(baseAmount)
                
                total = Double(baseAmount) + tipAmount!
                
            
        label.text = "total \(total), tip: \(tipAmount)."
